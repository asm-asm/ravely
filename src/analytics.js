...
export function analyticsRoutes({ env, directory }) {
  ...
  let buckets = {};
  if (enabled && fs.existsSync(file)) buckets = JSON.parse(fs.readFileSync(file, 'utf8'));
  let windowStart = Date.now(), requests = 0;
  router.post('/event', express.json({ limit: '2kb' }), (req, res) => {
  ...
  router.get('/summary', (req, res) => {
    const local = env.NODE_ENV !== 'production' && !token &&
    ...
    const days = Array.from({ length: 30 }, (_, i) => {
      ...
      return { date, ...(buckets[date] || { page_view: 0, search: 0, pattern_click: 0, tags: {} }) };
    });
